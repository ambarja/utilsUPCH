...
cp <- st_read(
  ...
  select(
    province,district,id_loc,
    village,year,month
    ,nrohab,fal,viv
    )

cp = read_rds("/Users/anton/Downloads/malaria_basins.rds")


defores <- def %>% 
  select(id_loc,HydroNameANA,HydroNameL6,HydroNameL7,RiverName,
         Adef_2009:Adef_2018) %>% 
  pivot_longer(!c("id_loc","HydroNameANA","HydroNameL6",
  "HydroNameL7","RiverName"),
  names_to = "year",
  values_to = "deforestation") %>% 
  mutate(year = gsub("Adef_","",year) %>% as.numeric()) %>% 
  # mutate(variable = "deforestation") %>% 
  slice(rep(1:n(), each = 12)) %>% 
  mutate(month = rep(1:12,18290)) %>% 
  mutate(year = as.numeric(year),
         month = as.numeric(month)) %>% 
  mutate(id_loc,HydroNameANA,HydroNameL6,HydroNameL7,RiverName,   
         year,month,deforestation)
...
GHM <- ghm %>% 
  ...
  mutate(year = as.numeric(year),
         month = as.numeric(month)) %>% 
  mutate(id_loc,HydroNameANA,HydroNameL6,HydroNameL7,RiverName,   
         year,month,global_human_mod) %>% 
    select(-c(gHM))


Humidity <- humidity %>% 
  select(id_loc,HydroNameANA,HydroNameL6,HydroNameL7,RiverName,
         `Qair_f_tavg2009-01`:`Qair_f_tavg2018-12`) %>% 
  pivot_longer(!c("id_loc","HydroNameANA","HydroNameL6",
                  "HydroNameL7","RiverName"),
               names_to = "year",
               values_to = "humidity",
...
tmax <- tmax %>%
  ...
  mutate(month = as.numeric(month),
         year = as.numeric(year)) %>% 
  mutate(id_loc,HydroNameANA,HydroNameL6,HydroNameL7,RiverName,   
         year,month,maximum_temperature)

tmin <- tmin %>%
  select(id_loc,HydroNameANA,HydroNameL6,HydroNameL7,RiverName,
         `tmmn2009-01`:`tmmn2018-12`) %>% 
  pivot_longer(!c("id_loc","HydroNameANA","HydroNameL6",
                  "HydroNameL7","RiverName"),
               names_to = "year",
  ...
  mutate(month = as.numeric(month),
         year = as.numeric(year)) %>% 
  mutate(id_loc,HydroNameANA,HydroNameL6,HydroNameL7,RiverName,   
         year,month,minimum_temperature)
...
db_total <- bind_rows(defores,ETP,GHM,Humidity,Pop,Pp,Ro,Soil,tmax,tmin)
...
data <- read_rds("data/db_varibles.rds")
final2 <- data %>% 
  select(province,district,village,HydroNameANA,HydroNameL6,HydroNameL7,RiverName,id_loc,   
         year,month,nrohab,fal,viv,deforestation,evapotranspiration,
         global_human_mod,humidity,population,precipitation,runoff,             
         soil_moisture,maximum_temperature,minimum_temperature) %>% 
  mutate(lon = st_coordinates(.)[,1],
         lat = st_coordinates(.)[,2]) %>% 
  st_drop_geometry()

write_sf(final,"data/db_variables.gpkg")
